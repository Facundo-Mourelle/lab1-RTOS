...
#define PERIODO_MS 500u
#define CARGA_MS 100u
...
void vTareaA(void *pvParameters) {
  ...
  for (;;) {
    ...
    /* 400ms de espera para intentar un período de 500ms */
    vTaskDelay(pdMS_TO_TICKS(PERIODO_MS - CARGA_MS));
  }
}
void vTareaB(void *pvParameters) {
  (void)pvParameters;
  ...
  for (;;) {
    ...
    /* siempre espera hasta el próximo múltiplo de 500ms */
    vTaskDelayUntil(&xLastWakeTime, pdMS_TO_TICKS(PERIODO_MS));
  }
}
/* USER CODE END 0 */
