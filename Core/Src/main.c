...
void vTareaB(void *pvParameters) {
  (void)pvParameters;
  uint8_t estadoAnterior = GPIO_PIN_RESET;
  uint8_t suspendida = 0;

  for (;;) {
    uint8_t estadoActual = HAL_GPIO_ReadPin(BTN_PORT, BTN_PIN);
    if (estadoActual == GPIO_PIN_SET && estadoAnterior == GPIO_PIN_RESET) {
      if (!suspendida) {
        vTaskSuspend(xHandleTareaA);
        suspendida = 1;
      } else {
        vTaskResume(xHandleTareaA);
        suspendida = 0;
      }
    }
...
int main(void) {
  ...
  MX_GPIO_Init();
  /* USER CODE BEGIN 2 */
  xTaskCreate(vTareaA, "LEDS", 128, NULL, 1, &xHandleTareaA);
  xTaskCreate(vTareaB, "BTN", 128, NULL, 2, NULL);
  /* USER CODE END 2 */
